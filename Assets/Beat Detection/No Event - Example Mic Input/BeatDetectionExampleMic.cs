	...
	********************************************/
	void Update() {
		...
		*/
		...
		if ((theBeat & (int)beatType.kick) != (int)beatType.None) {
			StartCoroutine(showText("kick"));
		}
		if ((theBeat & (int)beatType.snare) != (int)beatType.None) {
			StartCoroutine(showText("snare"));
		}
		if ((theBeat & (int)beatType.hithat) != (int)beatType.None) {
			StartCoroutine(showText("hitHat"));
		}
		if ((theBeat & (int)beatType.energy) != (int)beatType.None) {
			StartCoroutine(showText("energy"));
		}
	...
	private IEnumerator showText(string text) {
        ...
        toggleBool(text, true);
        yield return new WaitForSeconds(MIN_BEAT_SEPARATION * 3);
        toggleBool(text, false);

    }

    private void toggleBool(string text, bool onOff)
    {
        switch (text)
        {
            case "kick":
